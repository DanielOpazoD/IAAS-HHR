...
/**
 * SVG icon paths used throughout the app.
 * Each entry is a path `d` attribute for a 24x24 viewBox.
 */
const ICONS = {
  // Navigation & UI
...
interface IconProps {
  name: IconName
  className?: string
  /** Stroke width for the SVG paths. Default: 2 */
  strokeWidth?: number
}

/**
 * Reusable SVG icon component. Uses a centralized icon map
 * to eliminate inline SVG duplication across the codebase.
 *
 * @example
 * <Icon name="edit" className="w-4 h-4 text-gray-400" />
 * <Icon name="search" className="w-5 h-5" strokeWidth={1.5} />
 */
const Icon = memo(function Icon({ name, className = 'w-5 h-5', strokeWidth = 2 }: IconProps) {
  const d = ICONS[name]
  return (
